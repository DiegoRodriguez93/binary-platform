...
import { TrendingUp, TrendingDown, Activity, BarChart3, Target, Zap, Brain, AlertTriangle } from 'lucide-react';
import { CandlestickDataV3 } from './TradeV3';

interface MarketAnalysisV3Props {
    symbol: string;
...
const MarketAnalysisV3: React.FC<MarketAnalysisV3Props> = ({
    symbol,
    candlestickData,
    marketTrend,
    currentPrice
    ...
    const analysis = useMemo(() => {
        ...
        const recentCandles = candlestickData.slice(-25);
        ...
        const longMA = prices.slice(-12).reduce((sum, price) => sum + price, 0) / 12;
        const momentum = shortMA > longMA * 1.002 ? 'bullish' : shortMA < longMA * 0.998 ? 'bearish' : 'neutral';
        ...
        // Pattern detection
        const patterns = [];
        const lastCandles = recentCandles.slice(-4);
        ...
        if (lastCandles.length >= 1) {
            ...
            if (lowerShadow > body * 2 && upperShadow < body * 0.5) {
                patterns.push({ name: 'Hammer', signal: 'bullish', strength: 'medium' });
            }
        }
        ...
        if (rsi < 30 && momentum === 'bullish' && volumeRatio > 1.2) {
        ...
        } else if (momentum === 'bullish' && volumeRatio > 1.5) {
            signals.push({ type: 'buy', strength: 'medium', reason: 'Strong bullish momentum' });
        } else if (momentum === 'bearish' && volumeRatio > 1.5) {
            signals.push({ type: 'sell', strength: 'medium', reason: 'Strong bearish momentum' });
        }
        ...
        return {
            volatility: avgVolatility > 0.015 ? 'high' : avgVolatility > 0.008 ? 'medium' : 'low',
            momentum,
            support,
            resistance,
            rsi: Math.round(rsi),
    ...
    return (
        <div className="card-gradient backdrop-blur-lg border border-white/10 rounded-2xl p-6">
                            ...
                            <div key={index} className={`border rounded-lg p-3 ${
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className={`px-2 py-1 rounded-full text-xs ${
                                        signal.strength === 'strong' ? 'bg-purple-500/20 text-purple-400' : 'bg-gray-500/20 text-gray-400'
                                    }`}>
                                        {signal.strength}
                                    </span>
                                </div>
            ...
            <div className={`border rounded-lg p-4 ${
                ...
                <div className="text-sm text-gray-300">
                    {analysis.recommendation === 'buy' ? 
                        'Multiple bullish indicators detected. Market conditions favor higher trades.' :
                     analysis.recommendation === 'sell' ?
                        'Bearish signals confirmed. Market conditions favor lower trades.' :
                        'Mixed signals detected. Wait for clearer market direction or use smaller positions.'}
                </div>
            ...
            <div className="mt-4 text-xs text-gray-500 text-center bg-gray-800/20 rounded-lg p-3 border border-gray-700/30">
                ...
                <div>This analysis uses advanced algorithms and machine learning. Always do your own research and manage risk appropriately.</div>
            </div>
        </div>
    );
};

export default MarketAnalysisV3;